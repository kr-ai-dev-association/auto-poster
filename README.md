...
### 2. 마크다운-HTML 위키 변환
-   **다국어 동시 변환**: 하나의 마크다운 파일을 국문(`_ko.html`)과 영문(번역된 파일명) 버전의 HTML로 동시에 변환합니다.
-   **AI 요약 이미지 생성**: **Gemini 2.5 Flash Image** 모델을 사용하여 본문 내용을 시각화한 전문적인 기술 일러스트를 생성하고 타이틀 아래에 삽입합니다.
-   **정밀 이미지 가공**: 생성된 이미지를 중앙 기준으로 **16:9 비율**로 자동 크롭하여 위키 레이아웃에 최적화합니다.
-   **LaTeX 수식 및 반응형 레이아웃**: **MathJax v3**를 통한 수식 렌더링과 `@template.html` 기반의 완벽한 반응형 디자인을 제공합니다.
-   **자동 배포 시스템**: 변환 완료 후 생성된 HTML과 이미지를 절대 경로 `/Volumes/Transcend/Projects/tech-blog/html`로 자동 복사합니다.
