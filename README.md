...
유투브 영상 분석, 로고 삽입, 마케팅 최적화 설명 생성 및 자동 업로드를 수행합니다.

- **사전 준비**:
  1. Google Cloud Console에서 **YouTube Data API v3** 활성화 및 OAuth 클라이언트 ID(JSON) 다운로드.
  2. `3_youtube_poster/client_secrets.json`으로 저장.
  3. `v_source/` 폴더에 MP4, PDF, 로고(`banya_logo_qute.png`), 고정 설명(`desc.md`) 준비.
- **실행 방법**:
  ```bash
  python 3_youtube_poster/youtube_poster.py
  ```
- **주요 기능**:
  - **동영상 로고 삽입**: `ffmpeg`를 사용하여 우측 하단에 로고(기존 대비 20% 확대 적용) 자동 삽입.
  - **마케팅 최적화 메타데이터**:
    - **강력한 후킹**: Gemini AI가 시청자를 유혹하는 마케팅 문구와 이모지를 포함한 설명 생성.
    - **자동 해시태그**: 20개 이상의 트렌디한 해시태그 자동 생성 및 삽입.
    - **고정 정보 결합**: `desc.md`의 홍보 문구와 클릭 가능한 URL 링크를 하단에 자동 병합.
  - **다국어 지원**: 실행 시 한국어/영어 중 메타데이터 생성 언어 선택 가능.
  - **자동 업로드**: 분석부터 편집, YouTube 업로드(`tony@banya.ai`)까지 전 과정 자동화.
