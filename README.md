...
- **파이프라인 큐 시스템**: 여러 주제를 대기열에 등록하고 순차적으로 자동 생성
  - 대기열 추가/삭제, 시작/중지 (현재 항목 완료 후 graceful stop)
  - 실시간 큐 상태 폴링 (2초 간격), 진행 중 항목 진행률 기존 UI 연동
  - 페이지 새로고침 시 localStorage 기반 큐 상태 자동 복구
...
### 2026-02-11: 파이프라인 큐 시스템
- **큐 관리**: 여러 주제를 대기열에 등록하고 FIFO 순서로 순차 자동 생성
- **API 엔드포인트**: `/api/pipeline/queue/add`, `start`, `stop`, `status`, `DELETE /{item_id}`
- **Graceful Stop**: "현재 항목 완료 후 중지" 기능으로 안전한 큐 중단
- **상태 복구**: localStorage 기반 페이지 새로고침 시 큐 상태 자동 복구
- **UI**: 대기열 목록, 상태별 색상 표시, 완료 항목 접기/펼치기
